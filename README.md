...
**Note:** there are limits for this directive in reverse proxy configurations. See the detailed description of [`MDRequireHttps`](#mdrequirehttps).
...
**Note:** when using `ProxyPass` directive or `RewriteRule` to another host, `MDRequireHttps` may not apply the `HSTS` header successfully. This depends on where the redirection is going and what that host is. Simplified, the original request is rewritten and follows its own rules when reverse proxying. In such setups it is better to configure redirects and `HSTS` headers with specific rewrite rules and directives from [mod_headers](https://httpd.apache.org/docs/current/mod/mod_headers.html).
