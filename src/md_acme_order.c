...
#define ORDER_CTX_INIT(ctx, p, o, a, n, d, pf, cid, nb, na, r) \
    (ctx)->p = (p); (ctx)->order = (o); (ctx)->acme = (a); \
    (ctx)->name = (n); (ctx)->domains = d; (ctx)->profile = pf; \
    (ctx)->ari_cert_id = cid; (ctx)->notBefore = nb; (ctx)->notAfter = na; (ctx)->result = r
...
static apr_status_t on_init_order_register(md_acme_req_t *req, void *baton)
{
    ...
    if (ctx->notBefore)
        md_json_sets(ctx->notBefore, jpayload, "notBefore", NULL);
    if (ctx->notAfter)
        md_json_sets(ctx->notAfter, jpayload, "notAfter", NULL);
                                    ...
                                    const char *notBefore, const char *notAfter)
{
    ...
    ORDER_CTX_INIT(&ctx, p, NULL, acme, name, domains, profile, ari_cert_id, notBefore, notAfter, NULL);
                                  ...
                                  md_result_t *result, apr_pool_t *p)
{
    ...
    assert(MD_ACME_VERSION_MAJOR(acme->version) > 1);
    ORDER_CTX_INIT(&ctx, p, order, acme, NULL, NULL, NULL, NULL, NULL, NULL, result);
                                       ...
                                       md_result_t *result, apr_pool_t *p)
{
    ...
    assert(MD_ACME_VERSION_MAJOR(acme->version) > 1);
    ORDER_CTX_INIT(&ctx, p, order, acme, md->name, NULL, NULL, NULL, NULL, NULL, result);
